...
from PIL import Image, ImageDraw, ImageTk  # Updated to include ImageTk
import getpass
...
# Function to start capturing multiple screenshots
def start_screenshot_capture():
    ...
    # Continuously listen for 'Print Screen' key to take screenshots
    while is_capturing:
        if keyboard.is_pressed('print screen'):
            ...
            keyboard.wait('print screen')  # Wait for the next key press
...
logo = logo.resize((100, 100), Image.Resampling.LANCZOS)  # Replace ANTIALIAS with LANCZOS
